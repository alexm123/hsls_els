## PART 1: Traditional Measurement Invariance Modeling (CONTINUOUS) ##

# starting with the finalized configural model:
cfa_config <-  '
  group: ELS
...
# (table fit_tab_metric1 built at the end of the file)
...
# Freeing the variance for i1 leads to the largest chisq difference (table
# built at the end of the table; fit_tab_strict)
...
# chisq difference. See table at the end (fit_tab_strict2)
...
# Freeing the variance of i4 leads to the largest chisq difference. See 
# fit_table_strict3 at the end of the file
...
fit_tab_metric1 <- round(rbind(cbind(s_config$fit[fit_ind],
...
colnames(fit_tab_metric1) <- c("config", paste0("metric", c("", 1:2, 4:5)))
rownames(fit_tab_metric1)[8] <- "p LRT"
rownames(fit_tab_metric1)[9] <- "Chisq diff"
fit_tab_metric1[c(8, 9), 2] <- NA
...
fit_tab_metric2 <- round(rbind(cbind(s_metric2$fit[fit_ind],
                                      s_metric_21$fit[fit_ind],
                                      s_metric_24$fit[fit_ind],
                                      s_metric_25$fit[fit_ind]),
                                p_lrt_free2lambda, p_lrt_free1chisqdiff), 3)
colnames(fit_tab_metric2) <- paste0("metric_2", c("", 1, 4:5))
rownames(fit_tab_metric2)[8] <- "p LRT"
rownames(fit_tab_metric2)[9] <- "Chisq diff"
fit_tab_metric2[8:9,1] <- NA
...
# get the subset of indices to show
fit_tab_metric1 <- rnd(fit_tab_metric1[c(1:3, 8:9),], 2)
fit_tab_metric2 <- rnd(fit_tab_metric2[c(1:3, 8:9),], 2)
fit_tab_strict <- rnd(fit_tab_strict[c(1:3, 8:9),], 2)
fit_tab_strict2 <- rnd(fit_tab_strict2[c(1:3, 8:9),], 2)
fit_tab_strict3 <- rnd(fit_tab_strict3[c(1:3, 8:9),], 2)

rownames(fit_tab_metric1) <- rownames(fit_tab_metric2) <- 
  rownames(fit_tab_strict) <- rownames(fit_tab_strict2) <-
  rownames(fit_tab_strict3) <- 
  c("npar", "$\\chi^2$", "df", "p LRT", "$\\Delta \\chi^2$" )

colnames(fit_tab_metric1) <- c("Configural", "M0", "M1", "M2", "M3", "M4")
colnames(fit_tab_metric2) <- c("M0", "M1", "M2", "M3")
colnames(fit_tab_strict) <- c("M0", "M1", "M2", "M3", "M4")
colnames(fit_tab_strict2) <- c("M0", "M1", "M2", "M3")
colnames(fit_tab_strict3) <- c("M0", "M1", "M2")

# to get npar and df to appear as whole numbers
fit_tab_metric1[c(1,3),] <- as.numeric(fit_tab_metric1[c(1,3),])
fit_tab_metric2[c(1,3),] <- as.numeric(fit_tab_metric2[c(1,3),])
fit_tab_strict[c(1,3),] <- as.numeric(fit_tab_strict[c(1,3),])
fit_tab_strict2[c(1,3),] <- as.numeric(fit_tab_strict2[c(1,3),])
fit_tab_strict3[c(1,3),] <- as.numeric(fit_tab_strict3[c(1,3),])

fit_tab_metric1 <- 
  rbind("loadings released" = c("-", "-", "i1", "i2", "i4", "i5"), 
        fit_tab_metric1)
fit_tab_metric2 <- 
  rbind("loadings released" = c("i2", "i2, i1", "i2, i4", "i2, i5"), 
        fit_tab_metric2)
fit_tab_strict <- 
  rbind("variances released" = c("-", "i1", "i2", "i4", "i5"), fit_tab_strict)
fit_tab_strict2 <- 
  rbind("variances released" = c("i1", "i1, i2", "i1, i4", "i1, i5"), 
        fit_tab_strict2)
fit_tab_strict3 <- 
  rbind("variances released" =  c("i1, i2", "i1, i2, i4", "i1, i2, i5"), 
        fit_tab_strict3)


# save the formatted tables
saveRDS(fit_tab_metric1, "rds/fit_tab_metric1.rds")
saveRDS(fit_tab_metric2, "rds/fit_tab_metric2.rds")
